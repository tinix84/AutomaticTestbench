...
if __name__ == "__main__":
    ...
    dmmiout_board = 0
    dmmiout_address = 22
    dmmiout_protocol = 'GPIB'
    dmmiout_port = generic_port.PyVisaPort(dmmiout_protocol, dmmiout_address, dmmiout_board)
    iout_dmm = dmm.HP34401A(dmmiout_port)
    iout_dmm.connect()
    print "  --> gen id: `%s'" % iout_dmm.identify()
    iout_dmm.reset()
    time.sleep(1)
...
#    iin_max=vout_max*iout_max/vin_min*2
    ...
    iin_dmm.set_current_mode(250e-3,1e-5)
    iin_dmm.set_trigger_source_immediate() 
    
    iout_dmm.set_current_mode(250e-3,1e-5)
    iout_dmm.set_trigger_source_immediate() 
    ...
    for vin in vin_vect:
        ...
        if inp:
            ...
            for iout in iout_vect:
                ...
                vin_meas=float(pwrgen.get_P6Vchannel_measure_voltage())
                #vin_meas=float(vin_dmm.get_measure())
                iin_meas=float(iin_dmm.get_measure())
                time.sleep(.5)
                iout_meas=float(iout_dmm.get_measure())
                time.sleep(.5)
                ...
                print "Vset=%.3e, Iset=%.3e, Vin=%.3e, Iin=%.3e, Vout=%.3e, Iout=%.3e, Pin=%.3e, Pout=%.3e, eff=%.3e" \
                        %  (vin, iout, vin_meas, iin_meas, vout_meas, iout_meas, pwrin, pwrout, eff)
