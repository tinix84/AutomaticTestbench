...
def pulse_analisys(histo_data, frame_data, frame_min, frame_min_idx, frame_idx0):
    ...
    t0=idxvect[ind0][0]+frame_idx0
    t1=frame_min_idx+frame_idx0
    t2=idxvect[ind0+1][0]+frame_idx0  
    
    #print t0,t1,t2
    #print [histo_data[t0], histo_data[t1], histo_data[t2]]
    
    pkpar3pFWHM, time3pFWHM, par3pFWHM = parabolic_fit_3pt([t0,t1,t2],[histo_data[t0], histo_data[t1], histo_data[t2]],1,1)
    print pkpar3pFWHM
    return pkpar3pFWHM, high_ref, low_ref, mid_ref
...
def peak_fit (histo_data, target_distance, blind_time=100, frame_len=100, local_avg_sample=100):
    ...
    pkpar3pFWHM, high_ref, low_ref, mid_ref = pulse_analisys(histo_data, frame_data, frame_min, frame_min_idx, xmin)
    
    print pkpar3p1d, pkpar3p2d, pkpar3p3d
    print pkgauss1d+xmin, pkgauss2d+xmin, pkgauss3d+xmin
    
    return pkpar3p1d, pkpar3p2d, pkpar3p3d, pkgauss1d, pkpar3pFWHM
    
if __name__ == "__main__":
    
    from lrf_histogram import *
        
    target_distance = 900 #approximate distance of the target express in samples
    blind_time = 100 #beginning blind time to avoid target recognition due to cross coupling
    frame_len= 100 #length of a subset signal around the target
    local_avg_sample=100
    
    read_filename = '../Mx130320102941.txt'
    ...
    pkpar3p1d, pkpar3p2d, pkpar3p3d, pkgauss1d, pkpar3pFWHM = peak_fit(hist_float32, target_distance, blind_time, frame_len, local_avg_sample)  
