...
#generate the reports after file processing
def do_report_mx(filename, mx_cmd_info, target_pos = None, blind_time = 0):
    ...
    np_peak_loc_avg = np.mean(mx_cmd_info.Histogram.raw_data[0:7])
    np_peak_tg_pk_abs = np.min(mx_cmd_info.Histogram.raw_data)
    np_peak_pos_off3580 = np.argmin(mx_cmd_info.Histogram.raw_data)
