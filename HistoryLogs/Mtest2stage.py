...
def list_higherSNRtarget(self, mx_cmd_info, target_pos, blind_time):
    target_list = []
    SNR_list = []
    for item in self.AllTargetInfo:
        SNRtarget = self.AllTargetInfo[item].snr
        if blind_time < self.AllTargetInfo[item].peak_pos_off3580 :
            SNR_list.append(SNRtarget)
            target_list.append(item)
    print SNR_list
    print target_list
    np.sort(SNR_list)
    print SNR_list
    idx = np.argsort(SNR_list)
    sorted_target_list = np.take(target_list,idx)
...
def do_header_report_mx():
    ...
    header.append("T1_peak_pos_off3580")
    header.append("T1_peak_tg_pk_rel")
    header.append("T1_peak_loc_avg")
    header.append("T1_peak_tg_pk_abs")
    header.append("T1_snr")
    
    header.append("T2_peak_pos_off3580")
    header.append("T2_peak_tg_pk_rel")
    header.append("T2_peak_loc_avg")
    header.append("T2_peak_tg_pk_abs")
    header.append("T2_snr")

#    header.append("pkpar3p1d")
#    header.append("pkpar3p2d")
#    header.append("pkpar3p3d")
#    header.append("pkpar3pFWHM")
    header.append("pulse_width")
...
def do_report_mx(filename, mx_cmd_info, target_pos = None, blind_time = 0, frame_len= 100, local_avg_sample= 100):
    ...
    if target_pos is None:
        ...
        targetlist = list_higherSNRtarget(mx_cmd_info, target_pos, blind_time, frame_len)

    meas_list = []
    meas_list.append(filename)
    meas_list.append('target1')
...
#        meas_list=meas_list+[0, 0, 0, 0, 0]
    ...
    meas_list.append(mx_cmd_info.AllTargetInfo['target1'].peak_pos_off3580)
    meas_list.append(mx_cmd_info.AllTargetInfo['target1'].peak_tg_pk_rel)
    meas_list.append(mx_cmd_info.AllTargetInfo['target1'].peak_loc_avg)
    meas_list.append(mx_cmd_info.AllTargetInfo['target1'].peak_tg_pk_abs)
    meas_list.append(mx_cmd_info.AllTargetInfo['target1'].snr)
    try:
        meas_list.append(mx_cmd_info.AllTargetInfo['target2'].peak_pos_off3580)
        meas_list.append(mx_cmd_info.AllTargetInfo['target2'].peak_tg_pk_rel)
        meas_list.append(mx_cmd_info.AllTargetInfo['target2'].peak_loc_avg)
        meas_list.append(mx_cmd_info.AllTargetInfo['target2'].peak_tg_pk_abs)
        meas_list.append(mx_cmd_info.AllTargetInfo['target2'].snr)
    except IndexError:
        meas_list=meas_list+[0, 0, 0, 0, 0]
        
    pkpar3p1d, pkpar3p2d, pkpar3p3d, pkgauss1d, pkpar3pFWHM, pulse_width = pf.peak_fit(mx_cmd_info.Histogram.raw_data, target_pos, blind_time, frame_len, local_avg_sample)  
...
#    meas_list.append(pkpar3pFWHM)
    ...
    meas_list.append(pulse_width*1000)
